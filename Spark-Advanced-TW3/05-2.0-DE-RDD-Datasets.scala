// Databricks notebook source exported at Wed, 8 Jun 2016 17:20:07 UTC
// MAGIC %md
// MAGIC 
// MAGIC ![Wikipedia Logo](http://sameerf-dbc-labs.s3-website-us-west-2.amazonaws.com/data/wikipedia/images/w_logo_for_labs.png)
// MAGIC 
// MAGIC ### Analyzing the Wikipedia PageCounts with RDDs and Datasets -- With Spark 2.0 Dataset API conversion demos!
...
// issue here ... can you figure it out?
...
val pagecountsDS = pagecountsRDD.toDS
...
val enPagecountsDS = pagecountsParsedDS.filter(myObj => { myObj._1 == "en" })

// COMMAND ----------

pagecountsParsedDS.printSchema

// COMMAND ----------

val enPagecountsDS = pagecountsParsedDS.filter($"_1" === "en")
...
enPagecountsDS.select("_3").first()
...
enPagecountsDS.select(sum("_3")).show
...
// MAGIC %md Warning: Next 2 cells -- 1.6 API -- doesn't run in 2.0 preview (8 June)!
...
pagecounts3PartitionsDS.filter($"_1" === "en" ).select($"_3").select(sum($"_3")).collect
...
pagecounts3PartitionsDS.select("_1", "_3").groupBy("_1").count().show


// COMMAND ----------

// groupBy with Dataset implementing "count" on the groups:

pagecounts3PartitionsDS.select("_1", "_3").as[(String,Int)].groupByKey(_._1)
  .mapGroups( (k:String, g:Iterator[(String,Int)]) => g.size).show


// COMMAND ----------

// groupBy with Dataset implementing "sum" on the groups:

pagecounts3PartitionsDS.select("_1", "_3").as[(String,Int)].groupByKey(_._1)
  .reduceGroups( (p1,p2) => (p1._1, p1._2 + p2._2) ).show
...
// MAGIC %md NOW (2.0) -- that statement isn't true: notice the "toDF()" line is commented out below and produces the same results!
