// Databricks notebook source exported at Wed, 1 Jun 2016 22:59:15 UTC
...
val desktopViewsByDayOfWeekDF = pageviewsTimestampDF.filter("site = 'desktop'").groupBy(date_format(($"timestamp"), "E").alias("Day of week")).sum().withColumnRenamed("sum(requests)", "total requests").select(prependNumberUDF($"Day of week"), $"total requests").orderBy("UDF(Day of week)").toDF("DOW", "desktop_requests")
...
// MAGIC %md Now that we have the two DataFrames, let's merge them into one:

// COMMAND ----------

val allViewsByDayOfWeekDF = mobileViewsByDayOfWeekDF.join(desktopViewsByDayOfWeekDF, mobileViewsByDayOfWeekDF("DOW") === desktopViewsByDayOfWeekDF("DOW"))
display(allViewsByDayOfWeekDF)

// COMMAND ----------

// MAGIC %md Click on the bar chart icon again to convert the above table into a Bar Chart. Also, under the Plot Options, you may need to set the Keys as "DOW" and the values as "mobile requests".
