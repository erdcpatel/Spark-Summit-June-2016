// Databricks notebook source exported at Wed, 1 Jun 2016 22:53:41 UTC
...
// MAGIC %fs ls /mnt/wikipedia-readonly/pagecounts/staging_parquet_enx_only/
...
val pagecountsEnAllDF = sqlContext.read.parquet("/mnt/wikipedia-readonly/pagecounts/staging_parquet_enx_only/")
...
// MAGIC %md A Count action reads the entire DataFrame from disk, so it is a good way to benchmark how long a full disk read takes. Above we can see that the count action takes about 3 - 6 seconds to read the 3 Parquet files from S3.
...
// MAGIC %md Consider opening the <a href="https://spark.apache.org/docs/latest/api/scala/index.html#org.apache.spark.sql.DataFrame" target="_blank">DataFrame API docs</a> in a new tab to keep it handy as a reference. 
// MAGIC 
...
// MAGIC 
// MAGIC You can learn more expression operators on columns (like !==, &&, <=, etc) in the <a href="https://spark.apache.org/docs/latest/api/scala/index.html#org.apache.spark.sql.Column" target="_blank">Column API docs</a>.
